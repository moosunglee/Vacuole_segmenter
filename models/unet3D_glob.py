...
class Unet3D_glob(nn.Module):
    ...
    def __init__(self, feature_scale=1, trans_feature=32):
        ...
        # downsampling
        self.conv1 = UnetConv3D(1, feature_num[0], is_batchnorm=True) ## O : feature_num[0] * 64*64*48
        self.layer1 = ResBlock(layer_num[0], feature_num[0], feature_num[0]//2, feature_num[1], stride=2, padding=1) ## feature_num[1] * 32*32*24
        self.layer2 = ResBlock(layer_num[1], feature_num[1], feature_num[1]//2, feature_num[2], stride=2, padding=1) ## feature_num[2] * 16*16*12
        self.layer3 = ResBlock(layer_num[2], feature_num[2], feature_num[2]//2, feature_num[3], stride=2, padding=1) ## feature_num[3] * 8*8*6
        self.layer4 = ResBlock(layer_num[3], feature_num[3], feature_num[3]//2, feature_num[4], stride=2, padding=1) ## feature_num[4] * 4*4*3
        ...
        self.gcn5 = GCN(feature_num[4], self.trans_feature, k=(1, 1, 1))
    ...
    def forward(self, x):
        fm1 = self.conv1(x) ## O :feature_num[0] * 64*64*48
        fm2 = self.layer1(fm1)
        fm3 = self.layer2(fm2)
        fm4 = self.layer3(fm3)
        fm5 = self.layer4(fm4)
